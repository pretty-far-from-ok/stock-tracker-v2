...
class BarSeries extends Component {
	...
	render() {
		...
		return (
			<GenericChartComponent clip={clip} svgDraw={this.renderSVG} 
                canvasToDraw={getAxisCanvas} canvasDraw={this.drawOnCanvas} drawOn={["pan"]}/>
		);
	}
}
...
function getBars(props, moreProps) {
	...
	*/
	...
	const bars = plotData
		.filter(d => isDefined(yAccessor(d))).map(d => {
