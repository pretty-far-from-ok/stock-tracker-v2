...
class LineSeries extends Component {
	...
	drawOnCanvas(ctx, moreProps) {
		const {yAccessor, stroke, strokeOpacity, strokeWidth, hoverStrokeWidth, defined, 
            strokeDasharray, interpolation, canvasClip} = this.props;
		...
		const dataSeries = d3Line().x(d => Math.round(xScale(xAccessor(d)))).y(d => Math.round(yScale(yAccessor(d))));
