	...
	render() {
		...
		return (
			<g transform={translate}>
				<line x1={0} y1={2} x2={0} y2={14} stroke={this.props.color} strokeWidth="4px"/>
				<ToolTipText x={5} y={11}
					fontFamily={this.props.fontFamily} fontSize={this.props.fontSize}>
					<ToolTipTSpanLabel fill={labelFill}>{this.props.displayName}: </ToolTipTSpanLabel>
					<tspan fill={textFill}>{this.props.value}</tspan>
				</ToolTipText>
				<rect x={0} y={0} width={55} height={30} onClick={this.handleClick} fill="none" stroke="none" />
			</g>
		);
	}
}
...
MovingAverageTooltip.defaultProps = {
	...
	origin: [0, 10],
    // set width to adjust each options' gap 
	width: 90,
};
