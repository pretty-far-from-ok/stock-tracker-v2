...
class OHLCTooltip extends Component {
	...
	renderSVG(moreProps) {
		...
		const itemsToDisplay = {displayDate, open, high, low, close, percent, volume, x, y};

		return this.props.children(this.props, moreProps, itemsToDisplay);
	}
	render() {
		return (
			<GenericChartComponent clip={false} svgDraw={this.renderSVG} drawOn={["mousemove"]}/>
		);
	}
}
...
const displayTextsDefault = {d: "Date: ", o: " O: ", h: " H: ", l: " L: ", c: " C: ", v: " Vol: ", na: "n/a"};
...
function defaultDisplay(props, moreProps, itemsToDisplay) {
	...
	return (
        <g className={`react-stockcharts-tooltip-hover ${className}`} transform={`translate(${x}, ${y})`} onClick={onClick}>
			<ToolTipText x={0} y={0} fontFamily={fontFamily} fontSize={fontSize}>
				<ToolTipTSpanLabel fill={labelFill} key="label" x={0} dy="5">{displayTexts.d}</ToolTipTSpanLabel>
